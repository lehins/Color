...
-- TODO: move to VGamut.hs
-- data VRGB (i :: Illuminant2)

-- newtype instance Pixel (VRGB 'D65) e = VRGB (Pixel CM.RGB e)
--   deriving (Eq, Functor, Applicative, Foldable, Traversable, Storable)


-- instance (Elevator e) => ColorModel (VRGB 'D65) e where
--   type Components (VRGB 'D65) e = Components CM.RGB e
--   toComponents = toComponents . coerce
--   {-# INLINE toComponents #-}
--   fromComponents = coerce . fromComponents
--   {-# INLINE fromComponents #-}

-- instance RedGreenBlue VRGB 'D65 where
...
instance (Typeable i, Typeable k, Elevator e) => ColorModel (RGB (i :: k)) e where
  type Components (RGB i) e = Components CM.RGB e
  toComponents = toComponents . coerce
  {-# INLINE toComponents #-}
  fromComponents = coerce . fromComponents
  {-# INLINE fromComponents #-}
...
instance (Typeable k, Typeable i, Illuminant i, Elevator e) => ColorSpace (RGB (i :: k)) e where
  ...
  toBaseColorSpace = coerce
  fromBaseColorSpace = coerce
