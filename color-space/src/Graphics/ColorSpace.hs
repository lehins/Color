...
toPixelBinary :: ColorSpace cs e => Pixel cs e -> Pixel Y Bit
...
-- foldl2 ::
--      (Applicative f, Foldable f) => (c -> a -> b -> c) -> c -> f a -> f b -> c
-- foldl2 f acc px1 px2 =
--   foldl' (flip ($)) acc $ liftA2 (\x1 x2 acc' -> f acc' x1 x2) px1 px2
-- {-# INLINE foldl2 #-}

-- -- | Check weather all channels in two Pixels are equato each other up to a
-- -- tolerance. Useful for comparing pixels with `Float` or `Double` precision.
-- eqTolPx :: (Ord a, Num a, Applicative f, Foldable f) => a -> f a -> f a -> Bool
-- eqTolPx !tol = foldl2 comp True
--   where
--     comp !acc !e1 !e2 = acc && max e1 e2 - min e1 e2 <= tol
--     {-# INLINE comp #-}
-- {-# INLINE eqTolPx #-}
